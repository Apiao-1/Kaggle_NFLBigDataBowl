...
def make_pred(test, sample, env, model):
    ...
    # print("test_dense_players.shape:",test_dense_players.shape)
...
if __name__ == '__main__':
    ...
    train_dense_players = [train_dense[dense_player_features].iloc[np.arange(k, len(train), 22)].reset_index(drop=True)
                           for k in range(22)]
    ...
    print(train_dense_game.shape, train_dense_players.shape, train_cat_game.shape, train_cat_players.shape, train_y.shape)

    ## concat all features
    train_dense_players = np.reshape(train_dense_players, (len(train_dense_players), -1))
    print("test_dense_players.shape:",train_dense_players.shape) # (len,286)就是吧特征展开了，y*z
    ...
    models = []
    for k in range(1):
        kfold = KFold(5, random_state=0, shuffle=True) #k改变记得要更改random_state
    ...
    feature_importance_df = pd.DataFrame(np.vstack([feature_importances, feature_names]).T,
    ...
    sns.barplot(data=feature_importance_df.sort_values(by="importance", ascending=False).head(50), x="importance",
                y="name")
    plt.show()

    plt.figure(figsize=(8, 18))
    sns.barplot(data=feature_importance_df.sort_values(by="importance", ascending=False).tail(50), x="importance",
                y="name")
    plt.show()

    ## bad features
    list(feature_importance_df[
             feature_importance_df["importance"] < np.quantile(feature_importance_df["importance"], 0.3)]["name"])
