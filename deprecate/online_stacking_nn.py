...
def train_lgb(X, y, n_splits, single=False):
    ...
    predict_test = np.clip(np.cumsum(predict_test, axis=1), 0, 1).tolist()[0]
...
if __name__ == '__main__':
    ...
    boost_NN, predict_test_conduct = train_NN(X, y, 4, False, seed=43)
    models.append(boost_NN)
    print(models)
    for j, v in enumerate(clfs):
        clf, single_test = eval('train_%s' % v)(X, y, 4, False)
        models.append(clf)
        predict_test_conduct = np.concatenate((predict_test_conduct, single_test), axis=1)
        print(predict_test_conduct.shape)
        # print(predict_test_conduct)
    ...
    print('blending')
    final_models, _ = train_NN(predict_test_conduct, y, 4, False, seed=43)
    ...
    for (test_df, sample_prediction_df) in env.iter_test():
        ...
        y_conduct = y_conduct.reshape([1, len(y_conduct)])
        y_pred = np.mean([model.predict(y_conduct) for model in final_models], axis=0)
        y_pred = np.clip(np.cumsum(y_pred, axis=1), 0, 1).tolist()[0]
        #         print(len(y_pred))
        # y_pred = np.mean([model.predict(scaled_basetable) for model in final_models], axis=0)
        # y_pred = np.clip(np.cumsum(y_pred, axis=1), 0, 1).tolist()[0]
